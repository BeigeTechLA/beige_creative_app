...
- 2026-06-15 post-completion UI token follow-up: pending-card Accept uses `AppColors.shootAcceptButtonBackground` + `AppColors.shootAcceptButtonText`; Decline uses `AppColors.shootDeclineButtonBackground` + `AppColors.shootDeclineButtonText`. Verified by focused `shoots_screen_test.dart`.
